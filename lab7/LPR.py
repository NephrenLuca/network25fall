...
class ProjectController(app_manager.RyuApp):
    ...
    def get_lpr_path(self, src_dpid, dst_dpid):
        ...
        if key in self.lpr_paths:
            return self.lpr_paths[key]
    ...
    def _packet_in_handler(self, ev):
        ...
        # ---------- 路径计算：使用LPR算法 ----------
        path = self.get_lpr_path(src_edge, dst_edge)
        if not path:
            print(f"No path found from {src_edge} to {dst_edge}")
            return

        print(f"LPR Path for {src_ip} -> {dst_ip}: {path}")
        ...
        # 为目的边缘交换机安装反向路径的流表（如果还没有的话）
        # 这里可以优化，但为了简单起见，我们先安装单向流表
        # 在实际网络中，流量是双向的，但由于这是一个教学实验，我们只处理单向
